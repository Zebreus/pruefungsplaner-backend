  ...
  QString createInvalidToken(QString key = "") {
    ...
    jwt::builder tokenBuilder =
        jwt::create<QtJsonTraits>()
            ...
            .set_payload_claim(
                "pruefungsplanerRead",
                jwt::basic_claim<QtJsonTraits>(QString("falsch")))
            .set_payload_claim("pruefungsplanerWrite",
                               jwt::basic_claim<QtJsonTraits>(QString("true")));
...
TEST_F(BackendServiceTests, invalidTokensGetDetected) {
  ...
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with missing audience";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with wrong audience";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_expires_at(std::chrono::system_clock::now() +
                          std::chrono::seconds{3600})
          .set_payload_claim("pruefungsplanerRead",
                             jwt::basic_claim<QtJsonTraits>(QString("dfasf")))
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with wrong pruefungsplanerRead";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with missing pruefungsplanerRead";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerRead",
                             jwt::basic_claim<QtJsonTraits>(QString("true")))
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("fdgdsd")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with wrong pruefungsplanerWrite";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerRead",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with missing pruefungsplanerWrite";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "Backend service accepted expired token";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "BackendService accepted token with future issued at";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "Backend service accepted token with no expiration";
  ...
  tokenBuilder =
      jwt::create<QtJsonTraits>()
          ...
          .set_payload_claim("pruefungsplanerWrite",
                             jwt::basic_claim<QtJsonTraits>(QString("true")));
  EXPECT_FALSE(backend.login(signToken(tokenBuilder)))
      << "Backend service accepted token with no issued at";
...
TEST_F(BackendServiceTests, setSemestersSetsSemesters) {
  ...
  EXPECT_EQ(modifiedSemesters->toArray()[0].toObject()["name"].toString(),
            "testname239847895");
  QString token = createValidToken();
  EXPECT_TRUE(backend.login(token)) << "Token is " << token.toStdString();
  ASSERT_TRUE(backend.setSemesters(modifiedSemesters->toArray()));
  ASSERT_EQ(backend.getSemesters().toArray()[0].toObject()["name"].toString(),
            "testname239847895");
  // TODO do not check the id values of the json objects, because it is valid to
  // change them
