...
struct QtJsonTraits {
  // Type Specifications
  using value_type = QJsonValue;    // The generic "value type" implementation,
  ...
  // Translation between the implementation notion of type, to the
  // jwt::json::type equivilant
  ...
  static void object_for_each(
      const object_type& object,
      std::function<void(const string_type&, const value_type&)> function) {
    for (QJsonObject::const_iterator value = object.begin();
  ...
  static void array_for_each(const array_type& array,
                             std::function<void(const value_type&)> function) {
